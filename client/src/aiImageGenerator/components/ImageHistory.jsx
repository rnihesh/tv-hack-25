...
const ImageHistory = ({ images, loading, onLoadMore, pagination, onImageSelect }) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-12 transition-colors duration-300">
        <div className="flex flex-col items-center justify-center">
          ...
          <span className="text-gray-600 dark:text-gray-300 font-medium">Loading your creative history...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-12 text-center transition-colors duration-300">
        <div className="max-w-md mx-auto">
          ...
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-3">No creations yet</h3>
          <p className="text-gray-600 dark:text-gray-300">Start generating amazing AI images to build your creative collection.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 transition-colors duration-300">
      {/* Header */}
      <div className="bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20 px-8 py-6 border-b border-gray-200 dark:border-gray-700 rounded-t-2xl">
        <div className="flex items-center">
          <div className="w-10 h-10 bg-gradient-to-br from-violet-100 to-purple-100 dark:from-violet-800 dark:to-purple-800 rounded-xl flex items-center justify-center mr-4">
          ...
          <div>
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Your Gallery</h3>
            <p className="text-gray-600 dark:text-gray-300 text-sm">
              {pagination.total} creation{pagination.total !== 1 ? 's' : ''} in your collection
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {images.map((image) => (
            <div
              key={image._id}
              className="group cursor-pointer bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 rounded-2xl overflow-hidden hover:shadow-xl hover:scale-[1.02] transition-all duration-300 border border-gray-200 dark:border-gray-600"
        ...
        {pagination.hasNext && (
          <div className="mt-8 text-center">
            <button
              onClick={onLoadMore}
              disabled={loading}
              className="bg-gradient-to-r from-violet-600 to-purple-600 text-white py-3 px-8 rounded-xl hover:from-violet-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium"
            >
              {loading ? (
                <div className="flex items-center">

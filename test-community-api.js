...
async function testCommunityAPI() {
  try {
    ...
    const getResponse = await axios.get('http://localhost:4000/api/community/messages', {
      headers: {
    ...
    const postResponse = await axios.post('http://localhost:4000/api/community/messages', {
    ...
    const getResponse2 = await axios.get('http://localhost:4000/api/community/messages', {
      headers: {
        'Authorization': 'Bearer dummy-jwt-token-for-testing'
      }
    });
